...
export default function Index() {
  const {account, pathway_by_pk} = useLoaderData()

  console.log(pathway_by_pk)
      ...
      <GlobalPage
        ...
        header={{
          ...
          refinementDetails: {
            ...
            info: {
              img: {
                alt: `${pathway_by_pk.name}`,
                src: `https://avatars.bugsyaya.dev/50/${pathway_by_pk.id}`,
              },
              title: `${pathway_by_pk.name}`,
              // tags: [{link: `/pathway/${student_by_pk.pathway.id}`, tag: student_by_pk.pathway.name}],
            }
          },
        }}
      >
        <Details constraints={[{constraints_pathway : pathway_by_pk.pathway_constraint}, {module_prerequis: pathway_by_pk.contraints_prerequis_modules}]} />
      </GlobalPage>
    </div>
  );
}
