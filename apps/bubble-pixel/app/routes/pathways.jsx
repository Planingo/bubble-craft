import { Card, GlobalPage, Grid, Icon, ListPathway } from "@planingo/design-system";
...
export default function Index() {
  ...
  const {account, pathway} = useLoaderData()
  ...
  return (
    <div>
      ...
      >
        {isGrid ? <Grid>{pathway.map(p => <Card key={p.id} title={`${p.name}`} />)}</Grid> :
            <ListPathway data={pathway}/>
        }
      </GlobalPage>
    </div>
  );
