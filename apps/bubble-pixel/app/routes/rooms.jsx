import { Card, GlobalPage, Grid, Icon, ListProfessor, ListRoom } from "@planingo/design-system";
  ...
  const response = await fetch('https://hogwarts-school.caprover.cocaud.dev/v1/graphql', {
    ...
    body: JSON.stringify({
      query: `query roles($accountId: uuid!) {
        ...
        room(order_by: {max_seats: asc, name: asc}) {
          id
          name
          description
          max_seats
...
export default function Index() {
  ...
  const {account, room} = useLoaderData()
  ...
  return (
    <div>
      ...
      >
        {isGrid ? <Grid>{room.map(p => <Card key={p.id} title={`${p.name}`} />)}</Grid> :
            <ListRoom data={room}/>
        }
      </GlobalPage>
    </div>
