...
const Row = ({ index, data, style }) => {
    ...
    return <Link to={`/students/${data[index].id}`} style={style} className="no-underline text-bluebubblecraft p-1 m-1 flex gap-3 w-full justify-between items-center border-b-4 border-indigo-500 hover:bg-bluebubblecraft hover:bg-opacity-10">
        ...
        <div className="w-80">{data[index]?.apprenticeships_aggregate.aggregate.count}</div>
...
export const ListCompany = ({data}) => {
    ...
    <div className="flex gap-3 w-full justify-between bg-bluebubblecraft text-white p-2">
        <>
            ...
            <div className="w-40 text-sm flex items-center"><p className="mr-2">Description</p><Sort size="20" className="mr-2"/><FilterIcon size="20"/></div>
            <div className="w-120 text-sm flex items-center"><p className="mr-2">Nombre d'alternant</p><Sort size="20" className="mr-2"/><FilterIcon size="20"/></div>
            </>
